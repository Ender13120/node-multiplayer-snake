...
export default class GameView {
  ...
  showKillMessage(
    ...
    this.setKillMessageWithTimer(
      `<span style='color: ${killerColor}'>${killerName}</span> killed ` +
  ...
  showRanIntoWallMessage(playerName, playerColor, playerAddress, txHash) {
    this.setKillMessageWithTimer(
      `<span style='color: ${playerColor}'>${playerAddress}</span> successfully verified, txHash:<span style='color: ${playerColor}'>${txHash}</span> `
    );
  }

  showSuicideMessage(victimName, victimColor) {
    this.setKillMessageWithTimer(
      `<span style='color: ${victimColor}'>${victimName}</span> committed suicide`
    );
  }

  showVerified(notification, playerColor) {
    const notificationDiv = DomHelper.getNotificationsDiv();
    const formattedNotification =
      `<div><span class='time-label'>${new Date().toLocaleTimeString()} - </span>` +
      `<span style='color: ${playerColor}'>${notification}<span/></div>`;
    notificationDiv.innerHTML =
      formattedNotification + notificationDiv.innerHTML;
  }
  ...
  showPlayerStats(playerStats) {
    let formattedScores =
      '<div class="player-stats-header"><span class="image"></span>' +
    ...
    for (const playerStat of playerStats) {
      ...
      formattedScores +=
        `<div class='player-stats-content'><span class='image'>${playerImageElement}</span>` +
  ...
  _handleBackgroundImageUpload() {
    const uploadedBackgroundImageAsFile =
      DomHelper.getBackgroundImageUploadElement().files[0];
    if (uploadedBackgroundImageAsFile) {
      // Convert file to image
      const image = new Image();
      const self = this;
      image.onload = () => {
        self.backgroundImageUploadCallback(
          image,
          uploadedBackgroundImageAsFile.type
        );
      };
  ...
  _saveNewPlayerName() {
    ...
    if (
      ...
      playerName.length <= ClientConfig.MAX_NAME_LENGTH
    ) {
  ...
  _initEventHandling(
    ...
    // Player controls
    DomHelper.getChangeColorButton().addEventListener(
      "click",
      playerColorChangeCallback
    );
    DomHelper.getChangeNameButton().addEventListener(
      "click",
      this._handleChangeNameButtonClick.bind(this)
    );
    DomHelper.getPlayerNameElement().addEventListener(
      "blur",
      this._saveNewPlayerName.bind(this)
    );
    DomHelper.getImageUploadElement().addEventListener(
      "change",
      this._handleImageUpload.bind(this)
    );
    DomHelper.getClearUploadedImageButton().addEventListener(
      "click",
      this.imageUploadCallback
    );
    DomHelper.getBackgroundImageUploadElement().addEventListener(
      "change",
      this._handleBackgroundImageUpload.bind(this)
    );
    DomHelper.getClearUploadedBackgroundImageButton().addEventListener(
      "click",
      this.backgroundImageUploadCallback
    );
    DomHelper.getPlayOrWatchButton().addEventListener(
      "click",
      this._handlePlayOrWatchButtonClick.bind(this)
    );
    DomHelper.getToggleGridLinesButton().addEventListener(
      "click",
      toggleGridLinesCallback
    );
    DomHelper.getToggleSoundButton().addEventListener(
      "click",
      muteAudioCallback
    );
    DomHelper.getFullScreenButton().addEventListener(
      "click",
      DomHelper.toggleFullScreenMode
    );
    ...
    DomHelper.getIncreaseStartLengthButton().addEventListener(
      "click",
      startLengthChangeCallback.bind(
        this,
        ClientConfig.INCREMENT_CHANGE.INCREASE
      )
    );
    DomHelper.getDecreaseStartLengthButton().addEventListener(
      ...
      startLengthChangeCallback.bind(
        this,
        ClientConfig.INCREMENT_CHANGE.DECREASE
      )
    );
