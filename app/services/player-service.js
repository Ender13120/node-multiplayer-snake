...
class PlayerService {
  constructor(
    ...
    this.playerSpawnService = new PlayerSpawnService(
  ...
  addPlayer(socket, previousName, previousImage) {
    ...
    socket.emit(
      ServerConfig.IO.OUTGOING.NEW_PLAYER_INFO,
      playerName,
      newPlayer.color
    );
  ...
  changeColor(socket) {
    ...
    this.notificationService.broadcastNotification(
      `${player.name} has changed colors.`,
      newColor
    );
  }
      ...
      if (killReport.isSingleKill()) {
        ...
        if (killReport.killerId === killReport.victimId) {
          ...
          this.playerContainer
            .getPlayer(killReport.killerId)
            .grow(victim.getSegments().length);
  ...
  movePlayers() {
    for (const player of this.playerContainer.getPlayers()) {
      ...
      if (
        this.boardOccupancyService.isOutOfBounds(player.getHeadCoordinate()) ||
        ...
        this.notificationService.broadcastPlayerVerified(
          player.name,
          player.color
        );
  ...
  playerJoinGame(playerId) {
    ...
    this.notificationService.broadcastNotification(
      `${player.name} has rejoined the game.`,
      player.color
    );
  }
