...
class FoodService {
  ...
  consumeAndRespawnFood(playerContainer) {
    ...
    for (const foodConsumed of foodsConsumed) {
      const playerWhoConsumedFood = playerContainer.getPlayer(
        foodConsumed.playerId
      );
      ...
      if (
        food.type === ServerConfig.FOOD.SWAP.TYPE &&
        playerContainer.getNumberOfPlayers() > 1
      ) {
        const otherPlayer = playerContainer.getAnActivePlayer(
          playerWhoConsumedFood.id
        );
        this.boardOccupancyService.removePlayerOccupancy(
          otherPlayer.id,
          otherPlayer.getSegments()
        );
        this.boardOccupancyService.removePlayerOccupancy(
          playerWhoConsumedFood.id,
          playerWhoConsumedFood.getSegments()
        );
        ...
        otherPlayer.swapBodies(
          playerWhoConsumedFood.getSegments(),
          playerWhoConsumedFood.direction,
          playerWhoConsumedFood.directionBeforeMove,
          playerWhoConsumedFood.growAmount
        );
        playerWhoConsumedFood.swapBodies(
          otherPlayerSegments,
          otherPlayerDirection,
          otherPlayerDirectionBeforeMove,
          otherPlayerGrowAmount
        );

        this.boardOccupancyService.addPlayerOccupancy(
          otherPlayer.id,
          otherPlayer.getSegments()
        );
        this.boardOccupancyService.addPlayerOccupancy(
        ...
        this.notificationService.notifyPlayerFoodCollected(
          otherPlayer.id,
          "Swap!",
          playerWhoConsumedFood.getHeadCoordinate(),
          food.color,
          true
        );
      } else {
  ...
  generateSingleFood() {
    const randomUnoccupiedCoordinate =
      this.boardOccupancyService.getRandomUnoccupiedCoordinate();
    if (!randomUnoccupiedCoordinate) {
      this.notificationService.broadcastNotification(
        "Could not add more food.  No room left.",
        "white"
      );
      return;
    }
  ...
  getLastFoodIdSpawned() {
    return this.food[Object.keys(this.food)[Object.keys(this.food).length - 1]]
      .id;
  }
  ...
  removeFood(foodId) {
    ...
    this.boardOccupancyService.removeFoodOccupancy(
      foodId,
      foodToRemove.coordinate
    );
