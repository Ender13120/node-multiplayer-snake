...
class GameController {
  constructor() {
    ...
    this.botDirectionService = new BotDirectionService(
      this.boardOccupancyService
    );
    this.foodService = new FoodService(
      this.playerStatBoard,
      this.boardOccupancyService,
      this.nameService,
    ...
    this.playerService.init(
      this.adminService.getPlayerStartLength.bind(this.adminService)
    );
    this.initBlockchain();
  }
  ...
  listen(io) {
    ...
    io.sockets.on(ServerConfig.IO.DEFAULT_CONNECTION, (socket) => {
      socket.on(
        ServerConfig.IO.INCOMING.CANVAS_CLICKED,
        self._canvasClicked.bind(self, socket)
      );
      socket.on(
        ServerConfig.IO.INCOMING.KEY_DOWN,
        self._keyDown.bind(self, socket.id)
      );
      ...
      socket.on(
        ...
        self.playerService.playerSpectateGame.bind(
          self.playerService,
          socket.id
        )
      );
      ...
      socket.on(
        ...
        self.imageService.clearBackgroundImage.bind(
          self.imageService,
          socket.id
        )
      );
      socket.on(
        ...
        self.imageService.updateBackgroundImage.bind(
          self.imageService,
          socket.id
        )
      );
  ...
  async initBlockchain() {
    ...
    const privateKey = ""; // VERY BAD PRACTICE! NEVER DO!
  ...
  runGameCycle() {
    ...
    if (
      this.playerContainer.getNumberOfPlayers() -
        this.adminService.getBotIds().length ===
      0
    ) {
    ...
    setTimeout(
      this.runGameCycle.bind(this),
      1000 / this.adminService.getGameSpeed()
    );
  }
  ...
  _canvasClicked(socket, x, y) {
    ...
    if (this.boardOccupancyService.isWall(coordinate)) {
      ...
      this.notificationService.broadcastNotification(
        `${player.name} has removed a wall`,
        player.color
      );
    } else {
      this.boardOccupancyService.addWall(coordinate);
      this.notificationService.broadcastNotification(
        `${player.name} has added a wall`,
        player.color
      );
    }
  }
  ...
  _keyDown(playerId, keyCode) {
    GameControlsService.handleKeyDown(
      this.playerContainer.getPlayer(playerId),
      keyCode
    );
  }
}
